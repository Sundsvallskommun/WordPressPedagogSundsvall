...
$.fn.ofLoading = function(detach) {

    detach = (detach !== 'undefined') ? detach : false;

    if ($(this).attr('disabled') === 'disabled' && detach !== false)
        return;

    if (detach !== false) {
        var b = $('<b/>'),
            loader = $('<u/>').addClass('of-preloader');

        b.appendTo(loader);

        $(this).children().css('visibility', 'hidden');
        $(this).addClass('of-loading').prepend(loader);
        $(this).blur();
        return;
    }

    $(this).removeClass('of-loading').find('.of-preloader').remove();
