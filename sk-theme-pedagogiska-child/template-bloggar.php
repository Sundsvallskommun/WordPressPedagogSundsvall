				...
				<div class="sk-main sk-latest-posts-block list-blogs">
					<ul class="sk-grid-list">
						<?php foreach ( $sites as $site ) : ?>
							<li>
								<a href="<?php switch_to_blog( $site->blog_id ); ?><?php echo get_bloginfo( 'url' ); ?><?php restore_current_blog(); ?>"
								   class="of-dark-link">

									<?php if ( ! empty( $site->image ) ) : ?>
										<figure>
											<img width="<?php echo $site->image[1]; ?>"
											     height="<?php echo $site->image[2]; ?>"
											     alt="<?php echo $site->description; ?>"
											     class="attachment-thumbnail wp-post-image"
											     src="<?php echo $site->image[0]; ?>">
										</figure>
									<?php endif; ?>
