					...
					<?php if ( have_rows( 'campaign_content' ) ): ?>
						...
						<?php while ( have_rows( 'campaign_content' ) ): the_row();
							...
							<div class="item">
								...
								<img src="<?php echo $campaign_image['url']; ?>"
								     alt="<?php echo $campaign_image['alt'] ?>"/>
...
function get_latest_subsite_posts_box( $box ) {
	...
	$sites = get_sites( array( 'public' => 1, 'deleted' => 0 ) );
				...
				foreach ( $latest_posts->posts as $post ) :
					...
					<li>
						<a href="<?php echo $post->link; ?>">
							<div class="icon-circle-sm"><span title="<?php echo $label; ?>"
							                                  class="<?php echo $icon; ?>"></span></div>
...
function custom_breadcrumbs() {
	...
	if ( is_home() || is_front_page() ) {
		...
		if ( $showOnHome == 1 ) {
			echo '<div class="of-wrap breadcrumbs"><ul class="of-breadcrumbs"><li><a href="' . $homeLink . '">' . $home . '</a></li></ul>';
		}
